/*
 * sai.c
 *
 *  Created on: Jan 9, 2018
 *   Modified on: Mar 15, 2020
 *      Author: B50982, nxa32477
 */

#include "sai.h"
#include "dma.h"
#include "device_registers.h"

static void SAI_pins_init (void);
static void SAI_generate_bitClock (void);
static void SAI1_pins_init(void);

void SAI_i2s_init(void)
{
    /* Turn SAI0 clock on */
    PCC->PCCn[PCC_SAI0_INDEX] |= PCC_PCCn_CGC_MASK;

    /* Configure SAI0 pins */
    SAI_pins_init();

    /* Disable transmitter and receiver */
    SAI0->TCSR &= ~(SAI_TCSR_TE_MASK);
    SAI0->RCSR &= ~(SAI_RCSR_RE_MASK);
    /* Enable on debug mode and reset FIFO */
    SAI0->TCSR |= (SAI_TCSR_DBGE_MASK | SAI_TCSR_FR_MASK);
    SAI0->RCSR |= (SAI_RCSR_DBGE_MASK | SAI_RCSR_FR_MASK);

    /* Asyncrhonous mode for Transmitter, Receiver is configured in syncrhonous mode */
    SAI0->TCR2 &= ~SAI_TCR2_SYNC_MASK;
    SAI0->RCR2 = SAI_RCR2_SYNC(1);

    /* Clock polarity:
     * 0 - Bit clock is active high with drive outputs on rising edge and sample inputs on falling edge.*/
    SAI0->TCR2 &= ~SAI_TCR2_BCP_MASK;
    SAI0->TCR2 |= SAI_TCR2_BCP(0);

    /* Frame Sync Early:
     * 1 - Frame sync asserts one bit before the first bit of the frame */
    SAI0->TCR4 &= ~(SAI_TCR4_FSE_MASK);
    SAI0->TCR4 |= (SAI_TCR4_FSE(1));

    /* Frame Sync Polarity:
     * 0 - Frame sync is active high. */
    SAI0->TCR4 &= ~(SAI_TCR4_FSP_MASK);
    SAI0->TCR4 |= (SAI_TCR4_FSP(0));

    /* Frame Sync Width:
     * Configures the length of the frame sync in number of bit clocks.
     * The value written must be one less than the number of bit clocks*/
    SAI0->TCR4 &= ~(SAI_TCR4_SYWD_MASK);
    SAI0->TCR4 |= (SAI_TCR4_SYWD(15));

    /* Data order:
     * 1 - MSB is transmitted first. */
    SAI0->TCR4 &= ~(SAI_TCR4_MF_MASK);
    SAI0->TCR4 |= (SAI_TCR4_MF(1));

    /* Generate SYNC only when there is available data in TX_FIFO */
    SAI0->TCR4 |= SAI_TCR4_ONDEM_MASK;

    /* Slot offset:
     * Configures the bit index for the first bit transmitted for each word in the frame */
    SAI0->TCR5 &= ~(SAI_TCR5_FBT_MASK);
    SAI0->TCR5 |= SAI_TCR5_FBT(15);

    /* Slot size:
     * Configures the number of bits in each word and for first word independently */
    SAI0->TCR5 &= ~(SAI_TCR5_W0W_MASK | SAI_TCR5_WNW_MASK);
    SAI0->TCR5 |= SAI_TCR5_WNW(15) |
            SAI_TCR5_W0W(15);

    /* Slot count:
     * Configures the number of words in each frame.
     * The value written must be one less than the number of words in the frame*/
    SAI0->TCR4 &= ~(SAI_TCR4_FRSZ_MASK);
    SAI0->TCR4 |= (SAI_TCR4_FRSZ(1));

    /* Generate FS and BCLK as SAI is configured as Master */
    SAI_generate_bitClock();

    /* Enable 2 slots/channels in the audio frame (bit 0 and bit 1 are set to zero) */
    SAI0->TMR = 0xFFFC;

    /*Set watermark to 6*/
    SAI0->TCR1 = SAI_TCR1_TFW(6);

    /* Disable all SAI0 data lines */
    SAI0->TCR3 &= ~SAI_TCR3_TCE_MASK;

}

void SAI0_Enable_Tx(void)
{
    /* Enable DMA operation */
    SAI0->TCSR |=SAI_TCSR_FRDE_MASK;
    /* Enable DMA request */
    DMA_enableRequest(DMA_CHANNEL_Tx);
    /* Enable Transmit channel */
    SAI0->TCR3 |= SAI_TCR3_TCE(1 << 0);
    /* Enable SAI transmitter */
    SAI0->TCSR |= (SAI_TCSR_TE_MASK);

}


void SAI1_i2s_rx_slave_init(void)
{
    /* Turn SAI1 clock on */
    PCC->PCCn[PCC_SAI1_INDEX] |= PCC_PCCn_CGC_MASK;

    /* Configure SAI1 pins */
    SAI1_pins_init();

    /* Disable transmitter and receiver */
    SAI1->TCSR &= ~(SAI_TCSR_TE_MASK);
    SAI1->RCSR &= ~(SAI_RCSR_RE_MASK);

    /* Configure I2S format */
    SAI1->RCR4 = SAI_RCR4_FSE(1) |
            SAI_RCR4_FSP(0) |
            SAI_RCR4_SYWD(SAI_BITS_PER_CHANNEL - 1) |
            SAI_RCR4_MF_MASK|
            SAI_RCR4_FSD(0) |
            (SAI_RCR4_FRSZ(1));

    /* Enable on debug mode and reset FIFO */
    SAI1->TCSR |= (SAI_TCSR_DBGE_MASK | SAI_TCSR_FR_MASK);
    SAI1->RCSR |= (SAI_RCSR_DBGE_MASK | SAI_RCSR_FR_MASK);

    /* Configure bit width for every word */
    SAI1->RCR5 = SAI_RCR5_FBT(SAI_BITS_PER_CHANNEL - 1) |
            SAI_RCR5_WNW(SAI_BITS_PER_CHANNEL - 1) |
            SAI_RCR5_W0W(SAI_BITS_PER_CHANNEL - 1);

    /*  Generate bit clock externally (Slave mode) */
    SAI1->RCR2 &= ~SAI_RCR2_BCP_MASK;
    SAI1->RCR2 |= SAI_RCR2_BCP(0);

    /*Set RX Watermark to 1 */
    SAI1->RCR1 = SAI_RCR1_RFW(1);

    /* Enable 2 slots/channels in the audio frame (bit 0 and bit 1 are set to zero) */
    SAI1->RMR = 0xFFFC;

}

void SAI1_Enable_Rx(void)
{
    /* Enable DMA operation */
    SAI1->RCSR |= SAI_RCSR_FRDE_MASK;

    /* Enable DMA request for this channel */
    DMA_enableRequest(DMA_CHANNEL_Rx);

    /* Enable channel 0 */
    SAI1->RCR3 |= SAI_RCR3_RCE(1<<0);

    /* Enable SAI receiver*/
    SAI1->RCSR |= (SAI_RCSR_RE_MASK);
}

static void SAI_pins_init (void)
{
    /* Initialize clock for PORTA */
    PCC->PCCn[PCC_PORTA_INDEX] = PCC_PCCn_CGC_MASK;

    /* PTA13 as SAI0_D0 */
    PORTA->PCR[13] = PORT_PCR_MUX(7);
    /* PTA13 as SAI0_BCLK */
    PORTA->PCR[12] = PORT_PCR_MUX(7);
    /* PTA13 as SAI0_SYNC */
    PORTA->PCR[11] = PORT_PCR_MUX(6);

    /* Configure PDDR register for SAI pins */
    PTA->PDDR |= ((1 << 11) | (1 << 12) | (1 << 13));
}

static void SAI1_pins_init(void)
{
    /* Enable clock for PORTB and PORTE */
    PCC->PCCn[PCC_PORTB_INDEX] = PCC_PCCn_CGC_MASK;
    PCC->PCCn[PCC_PORTE_INDEX] = PCC_PCCn_CGC_MASK;

    /*SAI D0*/
    PORTB->PCR[9] = PORT_PCR_MUX(4);
    /*SAI BCLK*/
    PORTB->PCR[8] = PORT_PCR_MUX(4);
    /*SAI sync*/
    PORTE->PCR[2] = PORT_PCR_MUX(7);

    /* Configure PDDR register for SAI pins */
    PTB->PDDR &= ~((1 << 8) | (1 << 9));
    PTE->PDDR &= ~(1 << 2);
}

static void SAI_generate_bitClock (void)
{
    /* Bit clock is generated internally in Master mode.*/
    SAI0->TCR2 |= SAI_TCR2_BCD_MASK;
    /* Frame sync is generated internally in Master mode. */
    SAI0->TCR4 |= SAI_TCR4_FSD_MASK;

    /* Sample */
    /* BCLK = Bus clock / ((DIV + 1) * 2) */
    /* DIV = (SAI Clock / (128 * Sample Rate)) - 1*/
    SAI0->TCR2 &= ~(SAI_TCR2_MSEL_MASK | SAI_TCR2_DIV_MASK);
    SAI0->TCR2 |= (SAI_TCR2_MSEL(0) | SAI_TCR2_DIV(SAI_DIV_VALUE));
}

void SAI_i2s_fillTxBuffer (void)
{

    uint8_t fifoIndex = 0;
    uint16_t txBuffer[8] = {0xAA55, 0xAA55, 0xAA55, 0xAA55,
                            0xAA55, 0xAA55, 0xAA55, 0xAA55};

    /*Fill FIFO*/
    for (fifoIndex = 0; fifoIndex < 8; fifoIndex++)
    {
        SAI0->TDR[0] = txBuffer[fifoIndex];
    }
}

